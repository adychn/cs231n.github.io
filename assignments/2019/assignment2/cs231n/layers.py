...
def affine_backward(dout, cache):
    ...
    dw = X.T.dot(dout)
    db = np.sum(dout, axis=0) # so we just add up all biases' derivative per neuron layer.
...
def batchnorm_backward(dout, cache):
    ...
    dldvar = np.sum(dldz * (x - mean) * (-1/2) * (var)**(-3/2), axis=0) # D
    dldmean = np.sum(dldz * -1 / std, axis=0) + \
              dldvar * np.sum(-2 * (x - mean), axis=0) / m              # D
    dldx = dldz / std + \
           dldvar * 2 * (x - mean) / m + \
           dldmean / m                                                  # N x D
    dx = dldx
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    for i in range(N): # every image
        for j in range(F): # every output filter
            # sliding window
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    N, C, H, W = x.shape
    # reshape to fit batchnorm_forward function
    xtran = x.transpose((0, 2, 3, 1))
    xflat = xtran.reshape(-1, C)

    out, cache = batchnorm_forward(xflat, gamma, beta, bn_param)

    # reshape back to fit the original answer
    out = out.reshape(xtran.shape).transpose((0, 3, 1, 2))
...
def spatial_batchnorm_backward(dout, cache):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    N, C, H, W = dout.shape
    dout_tran = dout.transpose((0, 2, 3, 1))
    dout_flat = dout_tran.reshape(-1, C)  # X x C dim

    dx_flat, dgamma, dbeta = batchnorm_backward(dout_flat, cache)

    dx_tran = dx_flat.reshape(dout_tran.shape)
    dx = dx_tran.transpose((0, 3, 1, 2))
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def spatial_groupnorm_forward(x, gamma, beta, G, gn_param):
    ...
    for i in range(G):
        ...
        if out is None:
            ...
            out = np.vstack((out, out_group))

        cache.append(cache_group)
    cache.append(G)

    # reshape back to fit the original answer
    out = out.reshape(xtrans.shape).transpose((0, 1, 4, 2, 3)).reshape(N, C, H, W)
...
def spatial_groupnorm_backward(dout, cache):
    ...
    for i in range(G):
        ...
        if dx is None:
            ...
            dx = np.vstack((dx, dx_group))
            dgamma = np.hstack((dgamma, dgamma_group))
            dbeta = np.hstack((dbeta, dbeta_group))

    dx = dx.reshape(dout_trans.shape).transpose((0, 1, 4, 2, 3)).reshape(N, C, H, W)
